...
function eliminarMateria() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let url = "http://localhost:8080/classcraft/api/materia/deleteMateria";
...
function UpdateMateria() {
    let url = "http://localhost:8080/classcraft/api/materia/UpdateMateria";
...
function cargarMateria(page, pageSize) {
    ...
    const url = `http://localhost:8080/classcraft/api/materia/getAll?limit=${pageSize}&skip=${skip}`;
...
function mostrarDetalleMateria(idMateria) {
    const url = `http://localhost:8080/classcraft/api/materia/getMateriaDetails?id=${idMateria}`;
...
function ingresarMateria(event) {
    ...
    fetch("http://localhost:8080/classcraft/api/materia/insertMateria", requestOptions)
        .then(response => {
            if (!response.ok) {
                throw new Error('Error en la respuesta del servidor');
            }
