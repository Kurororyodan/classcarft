...
function eliminarSalon() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let url = "http://localhost:8080/classcraft/api/salon/deleteSalon";
...
function UpdateSalon() {
    let url = "http://localhost:8080/classcraft/api/salon/UpdateSalon";
...
function cargarSalon(page, pageSize) {
    ...
    const url = `http://localhost:8080/classcraft/api/salon/getAll?limit=${pageSize}&skip=${skip}`;
...
function mostrarDetalleSalon(idSalon) {
    const url = `http://localhost:8080/classcraft/api/salon/getSalonDetails?id=${idSalon}`;
...
function ingresarSalon(event) {
    ...
    fetch("http://localhost:8080/classcraft/api/salon/insertSalon", requestOptions)
        .then(response => {
            if (!response.ok) {
                throw new Error('Error en la respuesta del servidor');
            }
